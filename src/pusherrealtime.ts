// src/pusherrealtime.ts
/**
 * Core PusherRealtime implementation
 */
...
export class PusherRealtime {
    private config: PusherRealtimeConfig;
    private processed: number = 0;

    constructor(config: PusherRealtimeConfig = {}) {
        this.config = {
            verbose: false,
            timeout: 30000,
            maxRetries: 3,
    ...
    async execute(): Promise<ProcessResult> {
        ...
        try {
            if (this.config.verbose) {
                console.log('Initializing PusherRealtime processor...');
            }
